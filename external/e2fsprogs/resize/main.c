...
static void usage (char *prog)
{
	fprintf (stderr, _("Usage: %s [-d debug_flags] [-f] [-F] [-M] [-P] "
			   "[-p] [-R reserved_bytes] device [new_size]\n\n"), prog);
...
int main (int argc, char ** argv)
{
	...
	int		len, mount_flags;
	char		*mtpt;
	unsigned int	newgdt_count = 0;
	unsigned int	newgdt_blocks = 0;
	...
	while ((c = getopt (argc, argv, "d:fFhMPpS:R:")) != EOF) {
		switch (c) {
		case 'h':
			usage(program_name);
			break;
...
#define _SC_PAGESIZE _SC_PAGE_SIZE
	...
	reserved_blocks = ext2fs_div_ceil((unsigned int)reserved_bytes,
				fs->blocksize);
	if (new_size <= reserved_blocks) {
		fprintf(stderr, _("Error! Reserved bytes must be smaller than"
			"new file system length.\n\n"));
		exit(1);
	}
	new_size -= reserved_blocks;
	...
	/* new file system is too big. it will causes lack of reserved gdt */
	newgdt_count = ext2fs_div64_ceil(new_size,\
				EXT2_BLOCKS_PER_GROUP(fs->super));
	newgdt_blocks = ext2fs_div64_ceil(newgdt_count * EXT2_DESC_SIZE(fs->super),fs->blocksize);

	if( newgdt_blocks > fs->desc_blocks + fs->super->s_reserved_gdt_blocks ) {
		fprintf(stderr, _("Group Descriptor Table overflow."
			"It will lead to move bitmaps to other blocks."
			"We restrict this operation.\n"));
		exit(0);
	}
