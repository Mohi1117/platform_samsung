...
static void stlog(e2fsck_t ctx, char const *fmt, ...)
{
	int fd, len;
	char buffer[1024];
	char *pbuf;
	...
	if (access("/proc/stlog_version", F_OK) < 0)
		return;

	strcpy(buffer, "<e2fsck> ");
	len = strlen("<e2fsck> ");
	pbuf = buffer + len;
	...
	fd = open("/proc/stlog", O_RDWR);
	if ( fd < 0 )
		log_out(ctx, _("Can't open /proc/stlog(%s)\n"), strerror(errno));
	else {
		int ret = write(fd, buffer, len);
		if (ret < 0)
			log_out(ctx, _("Can't write /proc/stlog(%s)\n"), strerror(errno));
		close(fd);
	}
}
...
static void show_stats(e2fsck_t	ctx)
{
	...
	if (!verbose) {
		log_out(ctx, _("%s: %u/%u files (%0d.%d%% non-contiguous), "
			...
			blocks_used, blocks);
		stlog(ctx, ("%s: %u/%u files (%0d.%d%% non-contiguous), "
                               "%llu/%llu blocks\n"),
                        ctx->device_name, inodes_used, inodes,
                        frag_percent_total / 10, frag_percent_total % 10,
                        blocks_used, blocks);
...
static void check_if_skip(e2fsck_t ctx)
{
	...
	if (ctx->flags & E2F_FLAG_TIME_INSANE) {
		log_out(ctx, "E2F_FLAG_TIME_INSANE time(0) : %d\n", (int)ctx->now);
	...
	log_out(ctx, _("%s: clean, %u/%u files, %llu/%llu blocks"),
		...
		ext2fs_free_blocks_count(fs->super),
		ext2fs_blocks_count(fs->super));
	stlog(ctx, "%s: clean, %u/%u files, %llu/%llu blocks",
		ctx->device_name,
		fs->super->s_inodes_count - fs->super->s_free_inodes_count,
		fs->super->s_inodes_count,
		ext2fs_blocks_count(fs->super) -
...
skip:
	if(ext2fs_close(fs)) {
		log_out(ctx,_("ext2fs_close return error."
				" running e2fsck again would better than keep going\n"));
	}
	ctx->fs = NULL;
...
int main (int argc, char *argv[])
{
	...
	if (!(ctx->options & E2F_OPT_PREEN) || show_version_only) {
		log_err(ctx, "e2fsck %s (%s)\n", my_ver_string,
			 my_ver_date);
		stlog(ctx, "e2fsck %s (%s) [%s]\n", my_ver_string,
			 my_ver_date, my_basename(ctx->filesystem_name));
	}
...
failure:
	...
	if (sb->s_feature_incompat & EXT3_FEATURE_INCOMPAT_RECOVER) {
		...
		log_out(ctx, _("Skip journal recovery "
				"because INCOMPAT_RECOVER was clear.\n"));
	}
