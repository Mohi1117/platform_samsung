			...
			if (pass == PASS_SCAN &&
				...
				if (info->end_transaction) {
					journal->j_failed_commit =
						info->end_transaction;
					brelse(bh);
					printf("Journal recovery info->end_transaction : %u\n",
						  info->end_transaction);
					break;
				}
				...
				if (chksum_err) {
					...
					jbd_debug(1, "Checksum_err %x %x\n",
						  crc32_sum, found_chksum);
					printf("Journal checksum_err calc : 0x%x disk : 0x%x"
					       "info->end_transaction : %u block nr : %llu\n",
						  crc32_sum, found_chksum,
						  info->end_transaction, next_log_block - 1);
