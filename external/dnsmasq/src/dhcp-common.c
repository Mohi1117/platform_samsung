...
int iface_check_multicast_group (const char* iface_name, const char* mc_group)
{
  FILE* igmp6_file;
  const char* igmp6_file_path = "/proc/net/igmp6";
  char line[128];
  unsigned int iface_idx, iface_f_idx;
  char iface_f_name[IFNAMSIZ];
  struct in6_addr mc_group_addr;
  struct in6_addr mc_group_f_addr;
  int iface_joined = 0;
  int sscanf_rtv;
  if ( inet_pton(AF_INET6, mc_group, &mc_group_addr) != 1 ) {
    return -1;
  }
  if ( (iface_idx = if_nametoindex(iface_name)) == 0 ) {
    return -1;
  }
  if ( (igmp6_file = fopen(igmp6_file_path, "r")) == NULL ) {
    return -1;
  }
  while (fgets(line, sizeof(line), igmp6_file) != NULL) {
    sscanf_rtv = sscanf(line, "%4d %15s %08x%08x%08x%08x", &iface_f_idx, iface_f_name,
                        &mc_group_f_addr.s6_addr32[0], &mc_group_f_addr.s6_addr32[1],
                        &mc_group_f_addr.s6_addr32[2], &mc_group_f_addr.s6_addr32[3]);
    if (sscanf_rtv != 6) {
      break;
    }
    if (iface_idx != iface_f_idx) {
      continue;
    }
    mc_group_f_addr.s6_addr32[0] = ntohl(mc_group_f_addr.s6_addr32[0]);
    mc_group_f_addr.s6_addr32[1] = ntohl(mc_group_f_addr.s6_addr32[1]);
    mc_group_f_addr.s6_addr32[2] = ntohl(mc_group_f_addr.s6_addr32[2]);
    mc_group_f_addr.s6_addr32[3] = ntohl(mc_group_f_addr.s6_addr32[3]);
    if (mc_group_addr.s6_addr32[0] == mc_group_f_addr.s6_addr32[0] &&
        mc_group_addr.s6_addr32[1] == mc_group_f_addr.s6_addr32[1] &&
        mc_group_addr.s6_addr32[2] == mc_group_f_addr.s6_addr32[2] &&
        mc_group_addr.s6_addr32[3] == mc_group_f_addr.s6_addr32[3])
    {
      iface_joined = 1;
      break;
    }
...
void iface_join_multicast(const char* iface_name, const char* mc_group, int join)
{
    struct ipv6_mreq mreq;
    unsigned int if_index;
    if ((if_index = if_nametoindex(iface_name)) > 0) {
        mreq.ipv6mr_interface = if_index;
    ...
    const char* join_str = join ? "joined" : "left";
    my_syslog(MS_DHCP | LOG_INFO, "iface %s %s multicast group %s", iface_name, join_str, mc_group);
}
#endif
