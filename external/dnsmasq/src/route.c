...
//temp_block_ss extern int ifc_act_on_ipv6_route(int action, const char *ifname,
//temp_block_ss				 struct in6_addr dst, int prefix_length,
//temp_block_ss				 struct in6_addr gw);
...
//temp_block_ss    if (ifc_act_on_ipv6_route(SIOCADDRT, interface ,*dst, 128, gw.sin6_addr) < 0) {
//temp_block_ss        my_syslog(MS_DHCP | LOG_INFO, "failed to configure ipv6 routing table");
//temp_block_ss        return -1;
//temp_block_ss    }
...
//temp_block_ss            if (ifc_act_on_ipv6_route(SIOCADDRT, interface, *dst, 128, gw.sin6_addr) < 0) {
//temp_block_ss                my_syslog(MS_DHCP | LOG_INFO,"failed to delete routing table");
//temp_block_ss            }
            return -1;
        }
    }
...
void reactive_active_lists(struct in6_addr *dst, int prefix_len, char *interface)
{
    ...
    for (route = g_active_route; route; route = route->next) {
        if ((is_same_net6(dst, &route->dst, prefix_len)) &&
            (strcmp(interface, route->interface) == 0 ) &&
            (route->active == 0)) {
//temp_block_ss            if (ifc_act_on_ipv6_route(SIOCADDRT, route->interface, route->dst, 128, gw.sin6_addr) < 0) {
//temp_block_ss                my_syslog(MS_DHCP | LOG_INFO,"failed to delete routing table");
//temp_block_ss            }
...
//temp_block_ss            if (ifc_act_on_ipv6_route(SIOCDELRT, route->interface, route->dst, 128, gw.sin6_addr) < 0) {
//temp_block_ss                my_syslog(MS_DHCP | LOG_INFO,"failed to delete routing table");
//temp_block_ss            }
...
int modify_from_route(int table_num, char *action, char *addr, char *iface){
    char *cmd;
