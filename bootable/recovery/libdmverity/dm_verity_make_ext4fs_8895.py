...
reserve_min=1024*1024*32+40*1024
...
hash_postfix_size = 2 * 4096 # to cover blob size . this is to fix fec header start offset from the end .

fec_reserve_size  = 10 * 1024 * 1024
...
def calc_fec_rs_parity_size(dev_size):

    print 'Calculate Reed Solomon parity size based on device size'
    print 'dev size is ' + str(dev_size)
    blocks = math.ceil(dev_size / 4096)
    rounds = math.ceil(blocks / fec_rs_rsn)
    fec_rs_parity_size = 4096 * rounds * fec_rs_roots

    print "Calculated Reed Solomon parity size is " + str(fec_rs_parity_size)
    return fec_rs_parity_size    
...
def main():
    ...
    for o, a in optlist:
        if '-l' == o:
            length = int(a)
            reserve = calc_hashtree_size (length) + calc_fec_rs_parity_size(length) + fec_reserve_size
    ...
    cmd = ' '.join(['img_dm_verity', '/dev/block/platform/11120000.ufs/by-name/SYSTEM', str(length), image_file, image_file+'.tmp'])
    if(0 != run(cmd)):
        print 'failed!'
        return -1;
