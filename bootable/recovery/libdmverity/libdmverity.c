...
static const char * SVC_STATIC_STATUS = "init.svc.tz_service";
static const char * TZ_STATIC_DAEMON = "/sbin/tzdaemon_recovery";
#else
...
static const char * TZAPP_PARTITION = "/dev/block/platform/11120000.ufs/by-name/EFS";
#elif TRE_PROJECT
...
static const char * TZAPP_PARTITION = "/dev/block/platform/mtk-msdc.0/11230000.msdc0/by-name/efs";
#endif
#endif
...
static int file_to_device(const char * file, const char * dev, int buffer_size,
    ...
    int wfd = open(dev, O_WRONLY | O_SYNC);
    if (wfd < 0){
        printf("failed to open %s\n", dev);
        goto close_rfd;
    }
...
int dm_verity_verify(int mode) {
    int ret = -1;
    int verify_cnt = 2;

    sync();
    usleep(100*1000);
    dm_verity_drop_cache();
    usleep(100*1000);
...
int dm_verity_check_flag(void){
    return check_odin_flag();
}
