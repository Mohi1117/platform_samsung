...
int JackRequestDecoder::HandleRequest(detail::JackChannelTransactionInterface* socket, int type_aux)
{
    ...
    switch (type) {
		...
		case JackRequest::kStopJackd: {
            JackStopJackdRequest req;
            JackResult res;
            jack_log("JackRequest::StopJackd");
            CheckRead(req, socket);
            res.fResult = fServer->GetEngine()->ClientStopJackd(req.fRefNum, req.fParam);
            CheckWriteRefNum("JackRequest::StopJackd", socket);
            break;
        }

		case JackRequest::kSwitchMaster: {
            JackSwitchMasterRequest req;
            JackResult res;
            jack_log("JackRequest::SwitchMaster");
            CheckRead(req, socket);
            res.fResult = fServer->GetEngine()->ClientSwitchMaster(req.fRefNum, req.fType, req.fParam);
            CheckWriteRefNum("JackRequest::SwitchMaster", socket);
            break;
        }
#endif

#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
		case JackRequest::kStartDump: {
            JackStartDumpRequest req;
            JackResult res;
            jack_log("JackRequest::StartDump");
            CheckRead(req, socket);
            res.fResult = fServer->GetEngine()->ClientStartDump(req.fRefNum, req.fType, req.fParam);
            CheckWriteRefNum("JackRequest::StartDump", socket);
            break;
        }

		case JackRequest::kStopDump: {
            JackStopDumpRequest req;
            JackResult res;
            jack_log("JackRequest::StopDump");
            CheckRead(req, socket);
            res.fResult = fServer->GetEngine()->ClientStopDump(req.fRefNum, req.fParam);
            CheckWriteRefNum("JackRequest::StopDump", socket);
            break;
        }

		case JackRequest::kDump: {
            JackDumpRequest req;
            JackResult res;
            jack_log("JackRequest::Dump");
            CheckRead(req, socket);
            res.fResult = fServer->GetEngine()->ClientDump(req.fRefNum, req.fParam);
            CheckWriteRefNum("JackRequest::Dump", socket);
            break;
        }
#endif
