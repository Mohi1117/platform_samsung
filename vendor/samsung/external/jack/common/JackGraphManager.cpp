...
{
    //if ( (buffer_size > BUFFER_SIZE_MAX) || (buffer_size < 0)) {
    if ( buffer_size > BUFFER_SIZE_MAX ) { // fix warning
        jack_log("JackGraphManager::AssertBufferSize frames = %ld", buffer_size);
        assert((buffer_size <= BUFFER_SIZE_MAX) && (buffer_size >= 0));
    }
}

void JackGraphManager::AssertPort(jack_port_id_t port_index)
{
    //if ((port_index >= fPortMax) || (port_index < 0)) {
    if (port_index >= fPortMax ) { // fix warning
        jack_log("JackGraphManager::AssertPort port_index = %ld", port_index);
        assert((port_index < fPortMax) && (port_index >= 0));
    }
}
