...
int JackTimedDriver::FirstCycle(jack_time_t cur_time_usec)
{
    ...
    return int((double(fBufferSize) * 1000000) / double(fSampleRate));
}

int JackTimedDriver::CurrentCycle(jack_time_t cur_time_usec)
{
    return int(((double(fCycleCount) * double(fBufferSize) * 1000000.) / double(fSampleRate)) - (cur_time_usec - fAnchorTimeUsec));
}
