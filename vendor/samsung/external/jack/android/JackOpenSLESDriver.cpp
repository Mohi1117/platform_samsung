...
int JackOpenSLESDriver::Read() {
    ...
    if (inbuffer) {
        int samps = android_AudioIn(pOpenSL_stream,inbuffer,fBufferSize);
        ...
        for (int i = 0; i < fCaptureChannels; i++) {
            memset(GetInputBuffer(i), 0, sizeof(jack_default_audio_sample_t) * fBufferSize);  //silence
        }
    }
...
int JackOpenSLESDriver::Write() {
    ...
    if (outbuffer) {
        android_AudioOut(pOpenSL_stream, outbuffer, fBufferSize * 2);  //stereo output
        for (unsigned int i = 0, j = 0; i < fBufferSize; i++) {
