...
void JackAndroidThread::thread_exit_handler(int sig)
{
    ...
    jack_log("JackAndroidThread::thread_exit_handler sig = %d", sig);
    pthread_exit(0);
}
...
int JackAndroidThread::Kill()
{
    if (fThread != (jack_native_thread_t)NULL) { // If thread has been started
		...
		if(pthread_equal(fThread, pthread_self()) == 0 ){ // 0 is not equal
        	pthread_kill(fThread, SIGUSR1);
        	pthread_join(fThread, &status);
		} else {
			jack_log("kill was called in the same thread");
		}
        fStatus = kIdle;

        jack_log("JackAndroidThread::Kill : target thread killed");
...
int JackAndroidThread::Stop()
{
    if (fThread != (jack_native_thread_t)NULL) { // If thread has been started
        ...
        pthread_join(fThread, &status);
        jack_log("JackAndroidThread::Stop : target thread stoppted");
        fThread = (jack_native_thread_t)NULL;
        return 0;
    } else {
        return -1;
...
int JackAndroidThread::KillImp(jack_native_thread_t thread)
{
    if (thread != (jack_native_thread_t)NULL) { // If thread has been started
        jack_log("JackAndroidThread::KillImp");
        void* status;
		if(pthread_equal(thread, pthread_self()) == 0 ){ // 0 is not equal
			pthread_kill(thread, SIGUSR1);
			pthread_join(thread, &status);
		}
        jack_log("JackAndroidThread::KillImp : target thread killed");
        return 0;
    } else {
        return -1;
    }
...
int JackAndroidThread::StopImp(jack_native_thread_t thread)
{
    if (thread != (jack_native_thread_t)NULL) { // If thread has been started
        ...
        void* status;
        pthread_join(thread, &status);
        jack_log("JackAndroidThread::StopImp : target thread stoppted");
        return 0;
    } else {
        return -1;
    }
...
void JackAndroidThread::RequestPriority(int pid, int tid, int priority){
	int sock  = socket(AF_UNIX, SOCK_DGRAM, 0);

	if(-1 == sock) {
	  ALOGI("failed to create socket\n");
	  return;
	}
    ...
    if ( UNIX_PATH_MAX > strlen(SET_PRIORITY_SOCKET_NAME) ) {
        ...
        jack_error("fluid_requestPriority: Socket name length is longer than unix path max ");
        close(sock);
        return;
    }
	...
	if(aByte == -1) {
		ALOGE("fail to send a priority message[%s]", message);
	}
