...
namespace Jack
{
    ...
    JackSapaProxy::JackSapaProxy(jack_client_t* client, const JSList* params)
        :fClient(client)
#if defined (NEED_AUDIO_TUNING)        
        , audioFlinger(android::AudioSystem::get_audio_flinger())
#endif
    {
		...
		for( int i=0; i<SAPAPROXY_PORT_NUM_FOR_CLIENT; i++){
			fInputPorts[i] = NULL;
			fOutputPorts[i] = NULL;
		}
...
#if defined (NEED_AUDIO_TUNING) && (SEC_PRODUCT_FEATURE_AUDIO_SOUNDALIVE)
			// getCurDevice() is enable with SEC_PRODUCT_FEATURE_AUDIO_SOUNDALIVE
			if(sapaproxy->audioFlinger != NULL) {					
				if(sapaproxy->audioFlinger->getCurDevice() == 0x2){//audio_device_t AUDIO_DEVICE_OUT_SPEAKER = 0x2
					//to check speaker mode 
					//jack_log("JackSapaProxy::Speaker Mode");
