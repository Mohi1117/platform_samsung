...
#include "driver_interface.h"
    ...
    SERVER_EXPORT jack_driver_desc_t* jack_get_descriptor()
    {
        ...
        desc = jack_driver_descriptor_construct("in", JackDriverNone, "sapaproxy in client", &filler);
    ...
    {
        if (sapaproxy) {
            jack_info("sapaproxy in already loaded");
            return 1;
        }

        jack_log("Loading sapaproxy in");
        try {
            sapaproxy = new Jack::JackSapaProxy(jack_client, params);
        } catch (...) {
            jack_error("Failed to allocate sapaproxy in");
            sapaproxy = NULL;
            return -1;
        }
        assert(sapaproxy);
    ...
    SERVER_EXPORT void jack_finish(void* arg)
    {
        ...
        if (sapaproxy) {
            jack_log("Unloading sapaproxy in");
            sapaproxy = NULL;
            delete proxy;
        }
    }
