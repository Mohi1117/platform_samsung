...
static jack_time_t poll_time_playback = 0;
static jack_time_t poll_time_capture = 0;
#endif
...
#ifdef __ANDROID__
	if (driver->playback_2nd_addr) {
		free (driver->playback_2nd_addr);
		driver->playback_2nd_addr = 0;
	}

	if (driver->playback_2nd_interleave_skip) {
		free (driver->playback_2nd_interleave_skip);
		driver->playback_2nd_interleave_skip = NULL;
	}
#endif
...
#ifdef __ANDROID__
		|| handle == driver->playback_2nd_handle
#endif
	)
			    ...
			    jack_nframes_t rate)
{
	...
	if (driver->playback_2nd_handle) {
		if (alsa_driver_configure_stream (
			...
			if (driver->playback_2nd_handle){
				snd_pcm_close (driver->playback_2nd_handle);
				driver->playback_2nd_handle = 0;
			}
			jack_info("ALSA: cannot configure playback_2nd channel");
			jack_info("ALSA: It's normal case for headset or usb audio path with playback_2nd");
...
#ifdef __ANDROID__
	if (driver->playback_2nd_handle) {
		driver->playback_2nd_addr = (char **) malloc (sizeof (char *) * driver->playback_nchannels);
		memset (driver->playback_2nd_addr, 0, sizeof (char *) * driver->playback_nchannels);

		driver->playback_2nd_interleave_skip = (unsigned long *) malloc (sizeof (unsigned long *) * driver->playback_nchannels);
		memset (driver->playback_2nd_interleave_skip, 0, sizeof (unsigned long *) * driver->playback_nchannels);
	}
#endif
					...
					snd_pcm_uframes_t *playback_offset)
{
	...
	if (playback_avail) {
		if ((err = snd_pcm_mmap_begin (
			     driver->playback_2nd_handle, &driver->playback_2nd_areas,
			     (snd_pcm_uframes_t *) playback_offset,
			     (snd_pcm_uframes_t *) playback_avail)) < 0) {
			jack_error ("ALSA: playback_2nd: mmap areas info error");
			return -1;
		}

		for (chn = 0; chn < driver->playback_nchannels; chn++) {
			const snd_pcm_channel_area_t *a =
				&driver->playback_2nd_areas[chn];
			driver->playback_2nd_addr[chn] = (char *) a->addr
				+ ((a->first + a->step * *playback_offset) / 8);
			driver->playback_2nd_interleave_skip[chn] = (unsigned long ) (a->step / 8);
		}
	}
...
#ifdef __ANDROID__
	if (driver->playback_2nd_handle) {
		if ((err = snd_pcm_prepare (driver->playback_2nd_handle)) < 0) {
			jack_error ("ALSA: prepare error for playback on "
				    "\"%s\" (%s)", driver->alsa_name_playback,
				    snd_strerror(err));
			return -1;
...
#ifdef __ANDROID__
	if (driver->playback_2nd_handle) {
		driver->playback_2nd_nfds =
			snd_pcm_poll_descriptors_count (driver->playback_2nd_handle);
	} else {
		driver->playback_2nd_nfds = 0;
	}
#endif
...
#ifdef __ANDROID__
			(driver->playback_nfds + driver->playback_2nd_nfds + driver->capture_nfds + 3));
#else
...
#ifdef __ANDROID__
	if (driver->playback_2nd_handle) {
		if ((err = snd_pcm_start (driver->playback_2nd_handle)) < 0) {
			jack_error ("ALSA: could not start playback_2nd (%s)",
					snd_strerror (err));
			return -1;
		}
	}
...
#ifdef __ANDROID__
	if(driver->playback_2nd_handle){
		if ((err = snd_pcm_drop (driver->playback_2nd_handle)) < 0) {
			jack_error ("ALSA: channel flush for playback_2nd "
				    "failed (%s)", snd_strerror (err));
			return -1;
		}
	}
...
#ifdef __ANDROID__
		need_playback_2nd = driver->playback_2nd_handle ? 1 : 0;
#endif
	}
...
#ifdef __ANDROID__
		if (need_playback_2nd) {
			snd_pcm_poll_descriptors (driver->playback_2nd_handle,
						  &driver->pfd[nfds],
						  driver->playback_2nd_nfds);
			nfds += driver->playback_2nd_nfds;
		}
#endif
		if (need_capture) {
...
*/
		...
		if (need_playback_2nd) {
			int ret = -1;
			if (ret = snd_pcm_poll_descriptors_revents
				(driver->playback_2nd_handle, &driver->pfd[driver->playback_nfds],
				 driver->playback_2nd_nfds, &revents) < 0) {
				jack_error ("ALSA: playback_2nd revents failed [%d]", ret);
				*status = -6;
				return 0;
			}
	...
	if (driver->playback_handle && driver->capture_handle) {
		if (poll_time_playback > poll_time_capture)
			poll_time_diff = poll_time_playback - poll_time_capture;
		else
			poll_time_diff = poll_time_capture - poll_time_playback;
	}
#endif
	...
	if (driver->capture_handle) {
        ...
        if(capture_avail > driver->frames_per_cycle && capture_period_div % 2 == 0){         
            ...
            capture_avail -= capture_arrange;
        }
#endif
#if defined (__ANDROID__) && defined (WORKAROUND_SLSI_USBAUDIO_NOISE)
		if (poll_time_diff > (driver->period_usecs * 0.9)) {
			if (poll_time_playback > poll_time_capture) {
				/* capture first */
				jack_log ("trying to drop capture buffer(%u)", capture_avail);
				alsa_driver_read(driver, capture_avail);
			}
		}
#endif
	} else {
	...
	if (driver->playback_handle) {
        ...
        if(playback_avail > driver->frames_per_cycle && playback_period_div % 2 == 0){         
            ...
            playback_avail -= playback_arrange;
        }
#endif
	} else {
		/* odd, but see min() computation below */
...
#ifdef __ANDROID__
	jack_nframes_t nframes_2nd = nframes;
	snd_pcm_sframes_t nwritten_2nd = 0;
	snd_pcm_sframes_t contiguous_2nd = 0;
	snd_pcm_uframes_t offset_2nd;
#endif
...
*/
...
#if defined(__ANDROID__) && defined(ENABLE_RECOVERY_PTR_MISS_MATCHING)
		snd_pcm_t *playback_handle_logging = NULL;
	...
	if( driver->playback_2nd_handle ){
		while (nframes_2nd) {
			...
			contiguous_2nd = nframes_2nd;

			if (alsa_driver_get_channel_addresses_2nd ( driver, (snd_pcm_uframes_t *) &contiguous_2nd,&offset_2nd) < 0) {
				return -1;
			}

	        // JACK2
	        WriteOutput_2nd(orig_nframes, contiguous_2nd, nwritten_2nd);

			if ((err = snd_pcm_mmap_commit (driver->playback_2nd_handle,
					offset_2nd, contiguous_2nd)) < 0) {
				jack_error ("ALSA: could not complete playback_2nd of %"
					PRIu32 " frames: error = %d", contiguous_2nd, err);
				if (err != -EPIPE && err != -ESTRPIPE)
					return -1;
			}

			nframes_2nd -= contiguous_2nd;
			nwritten_2nd += contiguous_2nd;
		}
	}
...
#ifdef __ANDROID__
	if (playing_2nd) {
		if (snd_pcm_open (&driver->playback_2nd_handle,
				  playback_2nd_alsa_device,
				  SND_PCM_STREAM_PLAYBACK,
				  SND_PCM_NONBLOCK) < 0) {
			switch (errno) {
			case EBUSY:
...
#ifdef __ANDROID__
	if (driver->playback_2nd_handle == NULL) {
		if (playing_2nd) {
			jack_error ("ALSA: Cannot open PCM device %s for playback_2nd.", name);
			playing_2nd = FALSE;
		}
	}
#endif
