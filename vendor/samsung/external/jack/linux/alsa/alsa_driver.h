...
#ifdef __ANDROID__
    snd_pcm_t					 *playback_2nd_handle;
    char						**playback_2nd_addr;
    const snd_pcm_channel_area_t *playback_2nd_areas;
    unsigned long                *playback_2nd_interleave_skip;
    unsigned int                  playback_2nd_nfds;
#endif
...
#ifdef __ANDROID__
static inline void
alsa_driver_write_to_channel_2nd (alsa_driver_t *driver,
			      channel_t channel,
			      jack_default_audio_sample_t *buf,
			      jack_nframes_t nsamples)
{
	driver->write_via_copy (driver->playback_2nd_addr[channel],
				buf,
				nsamples,
				driver->playback_2nd_interleave_skip[channel],
				driver->dither_state+channel);
	alsa_driver_mark_channel_done (driver, channel);
}
#endif
