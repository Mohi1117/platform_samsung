...
#if defined(__ANDROID__) && defined(ENABLE_FAKE_PLAYBACK)
#include "JackFakePlayer.h"
#endif
...
#ifdef __ANDROID__
                         bool playing_2nd,
                         const char* playback_2nd_driver_name,
#endif
...
int JackAlsaDriver::Start()
{
    ...
    jack_info("jackAlsaDriver::Start res[%d]", res);
    return res;
}
...
int JackAlsaDriver::Stop()
{
    ...
    jack_info("jackAlsaDriver::Stop res[%d]", res);
    return res;
}
...
#ifdef __ANDROID__
			jack_default_audio_sample_t* buf;
			if(((alsa_driver_t *)fDriver)->playback_2nd_handle){
...
#ifdef __ANDROID__
void JackAlsaDriver::WriteOutputAux_2nd(jack_nframes_t orig_nframes, snd_pcm_sframes_t contiguous, snd_pcm_sframes_t nwritten){

    for (int chn = 0; chn < fPlaybackChannels; chn++) {
        // Output ports
        if (fGraphManager->GetConnectionsNum(fPlaybackPortList[chn]) > 0) {
            jack_default_audio_sample_t* buf = (jack_default_audio_sample_t*)fGraphManager->GetBuffer(fPlaybackPortList[0], orig_nframes);
            alsa_driver_write_to_channel_2nd(((alsa_driver_t *)fDriver), chn, buf + nwritten, contiguous);
        }
    }
}
...
#ifdef __ANDROID__
    jack_driver_descriptor_add_parameter(desc, &filler, "playback_2nd", 'K', JackDriverParamString, &value, NULL, "Provide playback_2nd ports.  Optionally set device", NULL);
#endif
    value.ui = 48000U;
...
SERVER_EXPORT Jack::JackDriverClientInterface* driver_initialize(Jack::JackLockedEngine* engine, Jack::JackSynchro* table, const JSList* params)
{
    ...
    int playback = FALSE;
#ifdef __ANDROID__
    int playback_2nd = FALSE;
    char *playback_2nd_pcm_name = NULL;
#endif
    int soft_mode = FALSE;
    int monitor = FALSE;
    ...
    int shorts_first = TRUE; // FALSE;   // in Android,  check format at first : 16 bits (TRUE) / 32 bits (FALSE)
        ...
        switch (param->character) {
            ...
            case 'K':
                if (strcmp (param->value.str, "none") != 0) {
					...
					if( playback_2nd_pcm_name != NULL ){
						free(playback_2nd_pcm_name);
						playback_2nd_pcm_name = NULL;
					}

                    playback_2nd_pcm_name = strdup (param->value.str);
                    jack_info("playback_2nd device %s", playback_2nd_pcm_name);
                }
				break;
#endif
            case 'D':
...
#ifdef __ANDROID__
					if( playback_2nd_pcm_name != NULL ) free(playback_2nd_pcm_name);
#endif
                    return NULL;
                }
                break;
    ...
    if (g_alsa_driver->Open(frames_per_interrupt, user_nperiods, srate, hw_monitoring, hw_metering, capture, playback, 
#ifdef __ANDROID__
						playback_2nd, playback_2nd_pcm_name,
#endif
						dither, soft_mode, monitor, user_capture_nchnls, user_playback_nchnls, shorts_first, capture_pcm_name, playback_pcm_name,
...
#ifdef __ANDROID__
void WriteOutput_2nd(jack_nframes_t orig_nframes, snd_pcm_sframes_t contiguous, snd_pcm_sframes_t nwritten)
{
    g_alsa_driver->WriteOutputAux_2nd(orig_nframes, contiguous, nwritten);
}
#endif
